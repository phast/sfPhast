...
class eCrop
{
	...
	/* Pasha's crystal method */
	public function cropCrystalArea($thumbDest, $free = true)
	{
		$this->createSourceImageResource();

		$ratio = $this->sourceWidth / $this->getThumbWidth();
		$cropHeight = round($this->getThumbHeight() * $ratio);
		if($cropHeight > $this->sourceHeight){
			$ratio = $this->sourceHeight / $this->getThumbHeight();
			$cropHeight = $this->sourceHeight;
		}
		$cropWidth = round($this->getThumbWidth() * $ratio);
		
		$this->setCropSize($cropWidth, $cropHeight);
		$this->setCropCoords(($this->sourceWidth - $this->getCropWidth()) / 2, ($this->sourceHeight - $this->getCropHeight()) / 2);
